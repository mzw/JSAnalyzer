...
//            }
            // Can have arguments, like return numbersonly(event);
            if (jsCode.startsWith("return") && (jsCode.endsWith(");") || jsCode.endsWith(")"))) {
            	jsCode = jsCode.substring("return".length(), jsCode.length()).trim();
            }
            
            // HTML event handler attributes can take value like "func(); return false;"
            // As well as above, remove "return false;" for preventing invalid parsing
            if(jsCode.endsWith("return false;")) {
            	jsCode = jsCode.substring(0, jsCode.length() - "return false;".length()).trim();
            }
            if(jsCode.endsWith("return false")) {
            	jsCode = jsCode.substring(0, jsCode.length() - "return false".length()).trim();
            }
            
            // To remove <!--  --> in JS code
//            if(jsCode.contains("<!--") && jsCode.contains("-->")) {
//            	jsCode = jsCode.replaceAll("<!--.+-->", "");
//            }
            
            // In jQuery, $("ID")[0] = document.getElementById("ID")
            // Remove "[0]" to get the "ID" simply
            jsCode = jsCode.replaceAll("\\$\\('#([a-zA-Z0-9_]+)'\\)\\[0\\]", "\\$\\('#$1'\\)");
